...
import MenuCardPlaylist from "./menu/menu";
import api from "../../../../../config/axiosConfig";

interface CardPlaylistType {
    name: string,
    type: string,
    duration: number,
    content_id: string,
...
export function Card({
    ...
    const handleRemoveContent = () => {
        api.delete(`playlist/${screen_id}/${content_id}`)
            .then(() => {
                onUpdate()
            })
    }
    ...
    return (
        ...
        >
            {type !== 'video/mp4' && type !== 'URL' && type !== 'HTML' ? <>
                <Box
                    component="img"
                ...
                >
                    {cardContent === false ? null : (
                        <TextField
                            ...
                            onChange={(e) => onSetDuration(content_id, Number(e.target.value))}
                            sx={{
                                width: "100px",
                            }}
                        />
